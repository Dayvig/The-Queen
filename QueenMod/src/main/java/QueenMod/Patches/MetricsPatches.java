...
import QueenMod.characters.TheQueen.Enums;
...
public class MetricsPatches {
    ...
    public static class SendPostPatch {
        ...
        public static void Prefix(Metrics metrics, @ByRef String[] url, String fileName) {
            if (AbstractDungeon.player.chosenClass == Enums.THE_QUEEN) {
                    url[0] = "http://queenmod.atwebpages.com";
            }
        }
    ...
    public static class RunPatch {
        ...
        public static void Postfix(Metrics metrics) {
            if (metrics.type == Metrics.MetricRequestType.UPLOAD_METRICS && AbstractDungeon.player.chosenClass == Enums.THE_QUEEN) {
                try {
