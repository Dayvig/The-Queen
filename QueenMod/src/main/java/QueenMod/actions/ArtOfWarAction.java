...
public class ArtOfWarAction extends AbstractGameAction {
    ...
    public ArtOfWarAction(AbstractCreature source, int amount, boolean endTurnDraw) {
    ...
    public ArtOfWarAction(AbstractCreature source, int amount) {
                ...
                if (deckSize + discardSize == 0) {
                    ...
                    if (this.amount != 0 && this.duration < 0.0F) {
                        ...
                        if (!AbstractDungeon.player.drawPile.isEmpty()) {
                            if (AbstractDungeon.player.drawPile.getTopCard().type.equals(AbstractCard.CardType.ATTACK)){
                                if (!AbstractDungeon.player.drawPile.getTopCard().freeToPlayOnce && AbstractDungeon.player.drawPile.getTopCard().costForTurn > 0) {
                                    AbstractDungeon.player.drawPile.getTopCard().costForTurn -= 1;
                                    AbstractDungeon.player.drawPile.getTopCard().isCostModifiedForTurn = true;
                                }
                            };
