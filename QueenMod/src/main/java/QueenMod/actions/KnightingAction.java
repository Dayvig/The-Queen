...
public class KnightingAction extends AbstractGameAction {
    ...
    static {
        uiStrings = CardCrawlGame.languagePack.getUIString("KnightingAction");
        TEXT = uiStrings.TEXT;
    }
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_FAST) {
            ...
            if (this.p.hand.group.size() == 1) {
                AbstractCard tmp = this.p.hand.getTopCard().makeStatEquivalentCopy();
                if (this.p.hand.getTopCard().upgraded){ tmp.upgrade(); }
                AbstractDungeon.player.masterDeck.addToHand(tmp);
                this.returnCards();
                this.isDone = true;
            }
